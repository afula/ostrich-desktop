...
class _MyHomePageState extends State<MyHomePage> with TrayListener,WindowListener{
  ...
  _dealServerListData() async{
    ...
    serverData.forEach((element) {
      menuItem.add(DropdownMenuItem(child: Text(element["country"]+"--"+element["city"]),value:element["country"]+"--"+element["city"],));
  ...
  _winStartOrClose() async {
      ...
      if (isRunning) {
        ...
        print("ostrich---start--");
        var result =  await nativeApi.leafRun(configPath: configPath,wintunPath: tunPath,tun2SocksPath: socksPath);
        print(result);
      }
...
//  检查代理连接情况
      ...
      appBar: AppBar(
        title: Row(
          ...
          children: [
            Text(widget.title),
            const Text("  "),
            Text(connectStatus,style: const TextStyle(fontSize: 12),)
          ],
        ),
        centerTitle: true,
        // leading: IconButton(
        ...
        child: Column(
          ...
          children:[
            Row(
              ...
              children: [
                Text(serverBabel,style: TextStyle(fontSize: 15,fontWeight: FontWeight.w500),),
                DropdownButton(value:chooseItem,items: serverDropMenuItem, onChanged: (value) async{
                  if(isConnected){
                    setState(() {
                      isConnected = false;
                    });
  ...
  Future<void> onTrayMenuItemClick(MenuItem menuItem) async {
   ...
   if(menuItem.label == "服务器列表"){
     ...
     if(Navigator.of(context).canPop() && CurrentPage.currentPage == "server_page"){
